...
const overdueData = [
  { studentName: "Jeremy", bookId: "B123", issueDate: "2025-01-01", returnDate: "2025-01-15" },
  { studentName: "Alex", bookId: "B124", issueDate: "2025-01-02", returnDate: "2025-01-16" },
];
...
export default function DashboardContent() {
  ...
  return (
    <div className="space-y-8 mt-[80px] p-8">
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="py-3 font-normal text-black text-lg">Username</th>
                  <th className="py-3 font-normal text-black text-lg">Department</th>
                </tr>
              </thead>
              <tbody>
                {userData.map((user, index) => (
                  <tr key={index} className="border-b border-gray-200">
                    <td className="py-3 text-black text-base font-normal">{user.matno}</td>
                    <td className="py-3 text-black text-base font-normal">{user.username}</td>
                    <td className="py-3 text-black text-base font-normal">{user.department}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {bookData.map((book, index) => (
                  <tr key={index} className="border-b border-gray-200">
                    <td className="py-3 text-black text-base font-normal">{book.matno}</td>
                    <td className="py-3 text-black text-base font-normal">{book.username}</td>
                    <td className="py-3 text-black text-base font-normal">{book.department}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {/* Overdue Book List */}
      <div className="bg-white rounded-2xl shadow-sm p-6">
        <h2 className="text-2xl font-bold text-black mb-4">Overdue Book List</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="py-3 font-normal text-black text-lg">Book Issue Date</th>
                <th className="py-3 font-normal text-black text-lg">Book Return Date</th>
              </tr>
            </thead>
            <tbody>
